...
void main() async {
  group('NIP-42', () {
    ...
    test('ensureAuthenticated sends AUTH without opening a REQ', () async {
      MockRelay relay1 = MockRelay(
        name: "relay 1",
        explicitPort: 3910,
        requireAuthForRequests: true,
      ...
      try {
        await ndk.relays.ensureAuthenticated(
          relayUrl: relay1.url,
          account: ndk.accounts.getLoggedAccount()!,
        );

        expect(relay1.authMessagesReceived, equals(1));
        expect(relay1.reqMessagesReceived, equals(0));
        expect(relay1.eventMessagesReceived, equals(0));
      } finally {
        await ndk.destroy();
        await relay1.stopServer();
      }
    ...
    test('ensureAuthenticated reuses pending and completed auth', () async {
      MockRelay relay1 = MockRelay(
        name: "relay 1",
        explicitPort: 3911,
        requireAuthForRequests: true,
      ...
      try {
        await Future.wait([
          ndk.relays.ensureAuthenticated(
            relayUrl: relay1.url,
            account: account,
          ),
          ndk.relays.ensureAuthenticated(
    ...
    test('ensureAuthenticated re-authenticates when account changes', () async {
      ...
      final account1 = ndk.accounts.getLoggedAccount()!;

      ndk.accounts.loginPrivateKey(
        pubkey: key2.publicKey,
        privkey: key2.privateKey!,
      );
      final account2 = ndk.accounts.getLoggedAccount()!;
      ...
      try {
        ...
        expect(relay1.authMessagesReceived, equals(3));
        expect(relay1.clientConnections, greaterThanOrEqualTo(3));
        expect(relay1.reqMessagesReceived, equals(0));
        expect(relay1.eventMessagesReceived, equals(0));
      } finally {
        await ndk.destroy();
    ...
    test(
      'ensureAuthenticated times out when relay omits required AUTH OK',
      () async {
        MockRelay relay1 = MockRelay(
          name: "relay 1",
          explicitPort: 3914,
    ...
    test(
      'ensureAuthenticated times out when relay never sends AUTH challenge',
      () async {
        MockRelay relay1 = MockRelay(
          name: "relay 1",
          explicitPort: 3912,
        ...
        try {
          ...
          expect(relay1.authMessagesReceived, equals(0));
          expect(relay1.reqMessagesReceived, equals(0));
        } finally {
          await ndk.destroy();
          await relay1.stopServer();
    ...
    test(
      'ensureAuthenticated authenticates the connection, not event authors',
      () async {
        final key2 = Bip340.generatePrivateKey();
        MockRelay relay1 = MockRelay(
          name: "relay 1",
        ...
        try {
          ...
          final eventFromDifferentPubkey = textNote(key2);
          final responses = await ndk.broadcast.broadcast(
            nostrEvent: eventFromDifferentPubkey,
            specificRelays: [relay1.url],
          ).broadcastDoneFuture;

          expect(responses.single.broadcastSuccessful, isTrue);
          expect(relay1.authMessagesReceived, equals(1));
          expect(relay1.eventMessagesReceived, equals(1));
        } finally {
          await ndk.destroy();
          await relay1.stopServer();
        }
